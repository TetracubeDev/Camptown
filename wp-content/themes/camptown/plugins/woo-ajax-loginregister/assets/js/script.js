...
jQuery(document).on("submit", '.woocommerce-form-register', function (e) {
	...
	if (reg_first_name.val() == '') {
		form.find(".register_msg.fail").text(zorem_ajax_object.required_message).show();
		showerror(reg_first_name);
		error = true;
	} else {
		hideerror(reg_first_name);
	}

	if (reg_last_name.val() == '') {
		form.find(".register_msg.fail").text(zorem_ajax_object.required_message).show();
		showerror(reg_last_name);
		error = true;
	} else {
		hideerror(reg_last_name);
	}
...
jQuery(document).on("keyup", '.woocommerce-form-register #reg_billing_first_name', function (e) {
	var form = jQuery('.woocommerce-form-register');
	var error;
	var reg_first_name = form.find("#reg_billing_first_name");
...
jQuery(document).on("keyup", '.woocommerce-form-register #reg_billing_last_name', function (e) {
	var form = jQuery('.woocommerce-form-register');
	var error;
	var reg_last_name = form.find("#reg_billing_last_name");
...
jQuery(document).on("submit", '.woocommerce-ResetPassword', function (e) {
	...
	if (!reset_key) {
		...
		if (lostpassword_captcha.val() == '') {
			...
			hideerror(lostpassword_captcha);
		}
...
function check_password(currentPass) {
	...
	if (currentPassVal.length < 8 ||
		!/\d/.test(currentPassVal) ||
		!/[A-Z]/.test(currentPassVal)) {
		res = false
