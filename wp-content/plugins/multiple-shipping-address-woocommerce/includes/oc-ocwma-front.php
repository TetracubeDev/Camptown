...
if (!class_exists('OCWMA_front')) {
  ...
  class OCWMA_front
  {
    ...
    function ocwma_Query_get($tablename, $type, $userid, $id = NULL, $count = NULL)
    {
      global $wpdb;
      if ($count == 1) {
    ...
    function ocwma_delete_Query_get($tablename, $delete_id)
    {
      global $wpdb;
...
?>
                ...
                <div class="address_line">
                  <div class="address_line_inner">
                    ...
                    <div class="address_select_button">
                      <a href="javascript:void(0)" class="choice_address" data-id="<?php echo esc_attr($row->id); ?>"><?php echo esc_html('Choice This Address'); ?></a>
                    </div>
                  </div>
                </div>
              <?php
                ...
                <div class="shipping_address_line">
                  <div class="shipping_address_line_inner">
                    ...
                    <div class="shipping_address_select_button">
                      <a href="javascript:void(0)" class="choice_shipping_address" data-id="<?php echo esc_attr($row->id); ?>"><?php echo esc_html('Choice This Address'); ?></a>
                    </div>
                  </div>
                </div>
              <?php
    ...
    function ocwma_my_account_endpoint_content()
    {
      ...
      $user_shipping = $this->ocwma_Query_get($tablename, 'shipping', $user_id);
      //$user_shipping = $wpdb->get_results( "SELECT * FROM {$tablename} WHERE type='shipping' AND userid=".sanitize_text_field($user_id));
      if ($ocwma_comman['ocwma_enable_multiple_shipping_adress'] == 'yes') {
    ...
    function ocwma_billing_popup_open()
    {
      ...
      if (empty($edit_id)) {
        ...
        $user = $this->ocwma_Query_get($tablename, 'billing', $user_id, 0, 1);
      ...
      ?>
        ...
        <div class="ocwma_modal-content">
          ...
          <?php
          //$user = $wpdb->get_results( "SELECT * FROM {$tablename} WHERE type='billing' AND userid=".sanitize_text_field($user_id)." AND id=".sanitize_text_field($edit_id));

          $user = $this->ocwma_Query_get($tablename, 'billing', $user_id, $edit_id);
    ...
    function ocwma_shipping_popup_open()
    {
      ...
      if (empty($edit_id)) {
        ...
        $user = $this->ocwma_Query_get($tablename, 'shipping', $user_id, 0, 1);
        $save_adress = $user[0]->count;
        $max_count = 3;
        if ($save_adress >= $max_count) {
          echo '<div class="ocwma_modal-content">';
        ...
        echo '<div class="ocwma_modal-content">';
        echo '<span class="ocwma_close">&times;</span>';
        // $user = $wpdb->get_results( "SELECT * FROM {$tablename} WHERE type='shipping' AND userid=".sanitize_text_field($user_id)." AND id=".sanitize_text_field($edit_id));
        $user = $this->ocwma_Query_get($tablename, 'shipping', $user_id, $edit_id);
        $user_data = unserialize($user[0]->userdata);
        $countries = new WC_Countries();
        if (!isset($country)) {
          $country = $countries->get_base_country();
    ...
    function ocwma_billing_data_select()
    {
      ...
      global $wpdb;
      $tablename = $wpdb->prefix . 'ocwma_billingadress';
      // $user = $wpdb->get_results( "SELECT * FROM {$tablename} WHERE type='billing' AND userid=".sanitize_text_field($user_id)." AND id=".sanitize_text_field($select_id));
      $user = $this->ocwma_Query_get($tablename, 'billing', $user_id, $select_id);
      $user_data = unserialize($user[0]->userdata);
    ...
    function ocwma_shipping_data_select()
    {
      ...
      global $wpdb;
      $tablename = $wpdb->prefix . 'ocwma_billingadress';
      //$user = $wpdb->get_results( "SELECT * FROM {$tablename} WHERE type='shipping' AND userid=".sanitize_text_field($user_id)." AND id=".sanitize_text_field($select_id));
      $user = $this->ocwma_Query_get($tablename, 'shipping', $user_id, $select_id);
      $user_data = unserialize($user[0]->userdata);
      echo json_encode($user_data);
      exit();
    }
    ...
    function OCWMA_all_billing_address()
    {
      ...
      if (is_user_logged_in()) {
        ...
        ?>
            ...
            ?>
              <a href="javascript:void(0)" class="choice_bil_address" style="background-color: <?php echo esc_attr($ocwma_comman['ocwma_btn_bg_clr']); ?>; color: <?php echo esc_attr($ocwma_comman['ocwma_font_clr']); ?>; padding: <?php echo esc_attr($ocwma_comman['ocwma_btn_padding']); ?>; font-size: <?php echo "15px" ?>;"><?php echo esc_html('Choice Billing Address'); ?></a>
            <?php
            }
            if ($ocwma_comman['ocwma_select_popup_btn_style'] == 'link') {
            ?>
              <a href="javascript:void(0)" id="choice_bil_address" class="choice_bil_address"><?php echo esc_html('Choice Billing Address'); ?></a>
          <?php
            }
          }
          ?>
      ...
      if (is_user_logged_in()) {
        if ($ocwma_comman['ocwma_enable_multiple_shipping_adress'] == 'yes') {
          ...
          ?>
            ...
            ?>
              <a href="javascript:void(0)" class="choice_sheep_address" style="background-color: <?php echo esc_attr($ocwma_comman['ocwma_btn_bg_clr']); ?>; color: <?php echo esc_attr($ocwma_comman['ocwma_font_clr']); ?>; padding: <?php echo esc_attr($ocwma_comman['ocwma_btn_padding']); ?>; font-size: <?php echo "15px" ?>;"><?php echo esc_html('Choice shipping Address'); ?></a>
            <?php
            }
            if ($ocwma_comman['ocwma_shipping_select_popup_btn_style'] == 'link') {
            ?>
              <a href="javascript:void(0)" id="choice_sheep_address" class="choice_sheep_address"><?php echo esc_html('Choice shipping Address'); ?></a>
          <?php
            }
          }
          ?>
          <button class="form_option_shipping" data-id="<?php echo esc_attr($user_id); ?>" style="background-color: <?php echo esc_attr($ocwma_comman['ocwma_btn_bg_clr']); ?>; color: <?php echo esc_attr($ocwma_comman['ocwma_font_clr']); ?>; padding: <?php echo esc_attr($ocwma_comman['ocwma_btn_padding']); ?>; font-size: <?php echo "15px" ?>;"><?php echo esc_html('Add Shipping Address'); ?></button>
<?php
        }
      }
    }
    ...
    function OCWMA_save_options()
    {
      ...
      if (isset($_REQUEST['action']) && $_REQUEST['action'] == "delete_ocma") {
        ...
        $this->ocwma_delete_Query_get($tablename, $delete_id);
        // $wpdb->query($sql);
